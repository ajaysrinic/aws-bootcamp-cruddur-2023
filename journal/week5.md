...
## Data Modelling
A data modeling technique called single table design stores all relevant data in a single database table. For the Direct Messaging System in our Cruddur application, we use DynamoDB. Four patterns of data access can be distinguished in this context:

`Pattern A` For displaying messages. A list of messages that are part of a message group is visible to users.

`Pattern B` For displaying message groups, Users can check the other people they have been communicating with by viewing a list of messaging groups.

`Pattern C` For composing a fresh message in a fresh message group

`Pattern D` For adding a new message to an existing message group

So, there are three types of items to insert in our Dynamo DB Table:
```
my_message_group = {
